...
if ($_SESSION['sessionRole']==1 || $_SESSION['sessionRole']==2){
  ...
  echo <<<"FORM"
    ...
    <form id="billing_form" class="homeForm" action="" method="post">
  FORM;
  ...
  if (isset($full_name)) {
    ...
    if (!mysqli_stmt_prepare($stmt,$sql)){
      ...
      while ($row = mysqli_fetch_array($result)) {
        if($row['days']) {
          $days = $row['days'];
          echo "<tr class='table-data'><td>Days</td><td>";
          echo $days."</td>";
          $total_days_billing = $days * 10;
          echo "<td>Total Days:</td><td> $".$total_days_billing."</td></tr>";
  ...
  if (!mysqli_stmt_prepare($stmt,$sql)){
    ...
    while ($row = mysqli_fetch_array($result)) {
      if($row['total']) {
        ...
        echo "<tr class='table-data'><td>Appointments</td><td>";
        echo $total_appts."</td>";
        $total_appts_billing = $total_appts * 50;
        echo "<td>Total Appt:</td><td> $".$total_appts_billing."</td></tr>";
  ...
  if (!mysqli_stmt_prepare($stmt,$sql)){
    ...
    while ($row = mysqli_fetch_array($result)) {
      if($row['total_morn']) {
        $morn = $row['total_morn'];
        echo "<tr class='table-data'><td>AM meds</td><td>";
        echo $morn."</td></tr>";
      } else {
      ...
      if($row['total_aft']) {
        $aft = $row['total_aft'];
        echo "<tr class='table-data'><td>PM meds</td><td>";
        echo $aft."</td></tr>";
      } else {
      ...
      if($row['total_night']) {
        ...
        echo "<tr class='table-data'><td>Night meds</td><td>";
        echo $night."</td></tr>";
      } else {
        echo "<tr><td colspan='2'>No Night meds</td></tr>";
      }
      $total_meds = ($morn + $aft + $night) * 5;
      echo "<tr class='table-data'><td></td><td></td><td>Total Meds:</td><td> $".$total_meds."</td></tr></table>";
    mysqli_stmt_close($stmt);

    if(isset($total_days_billing)) {
      $grand_total = $total_meds+$total_days_billing+$total_appts_billing;
      echo "<h2> Total Billing: $".$grand_total."</h2>";
    } else {
      $grand_total = $total_meds+$total_appts_billing;
      echo "<h2> Total Billing: $".$grand_total."</h2>";
    }
  }
  }
  ...
  echo<<< "FORM"
      ...
      <input type="number" min="0.00" step="0.01" name="amt_paid" placeholder="0.00">
